...
function dhinline.LoadCurrentTheme()
	local sThemeFromConvar = dhinline.GetVar("dhinline_core_theme", true)
...
/*
function dhinline.CreateSmoother(strName, numInit, numRate)
...
function dhinline.ChangeSmootherCurrent(strName, numCurrentCall)
	if not dhinline_dat.STOR_Smoothers[strName] then print("> " .. DHINLINE_NAME .. " Smoother ERROR : ChangeSmootherCurrent has requested field " .. strName .." which hasn't been created !") return end
	if type(numCurrentCall) == "table" then
		for k,v in pairs(numCurrentCall) do
			dhinline_dat.STOR_Smoothers[strName][2][k] = numCurrentCall[k]
		end
	end
end

function dhinline.ChangeSmootherTarget(strName, numTarget)
	if not dhinline_dat.STOR_Smoothers[strName] then print("> " .. DHINLINE_NAME .. " Smoother ERROR : ChangeSmootherTarget has requested field " .. strName .." which hasn't been created !") return end
	dhinline_dat.STOR_Smoothers[strName][1] = numTarget
end

function dhinline.ChangeSmootherRate(strName, numRate)
	if not dhinline_dat.STOR_Smoothers[strName] then print("> " .. DHINLINE_NAME .. " Smoother ERROR : ChangeSmootherRate has requested field " .. strName .." which hasn't been created !") return end
	dhinline_dat.STOR_Smoothers[strName][3] = numRate
end

function dhinline.GetSmootherCurrent(strName)
	if not dhinline_dat.STOR_Smoothers[strName] then print("> " .. DHINLINE_NAME .. " Smoother ERROR : GetSmootherCurrent has requested field " .. strName .." which hasn't been created !") return nil end
...
/*
function dhinline.RecalcAllSmoothers()
...
function dhinline.DeleteAllSmoothers()
	dhinline_dat.STOR_Smoothers = {}
end
*/
/////////////////////////////////////////////////////////////////////////
