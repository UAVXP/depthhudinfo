	...
	for k,ELEMENT in pairs( self.Elements ) do
		...
		if (
			...
			if ELEMENT.Think then
				local bOkay, strErr = pcall(ELEMENT.Think, ELEMENT)
				if not bOkay then print(" > " .. DHINLINE_NAME .. " Think ERROR on element ["..elementRawName.."] : ".. strErr) end
			end
			...
			if DHINLINE_BENCHMARK then print(elementRawName .. " took " .. ((SysTime() - bench)*1000) .. " ms.") end
		end
	end
	
	if DHINLINE_BENCHMARK then print("Done painting.") end
end
